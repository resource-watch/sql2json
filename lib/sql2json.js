...
const postgisFunctions = /ST_Intersects|ST_AsGeoJson|ST_SetSRID|ST_GeomFromGeoJSON|ST_METADATA|ST_SUMMARYSTATS|ST_HISTOGRAM|TO_NUMBER|TO_CHAR|ST_GeoHash|first|last|ST_BANDMETADATA|st_centroid|ST_X|ST_Y/gi;
...
sql2json.prototype.parseFunction = function (functionName) {
    const stack = [];
    let findParen = false;
    while (this.hasNext()) {
        ...
        switch (obtainType(token)) {
            ...
            case 'LAST':
                if (containAs) {
                    ...
                    this.parsed.select.push(this.parseFunction(token[1]));
                }
                break;
            case 'DISTINCT':
                isDistinct = true;
