...
const supportedFunctions = new RegExp(`^(${supportedFunctionList.join('|')}|CF_[a-zA-Z0-9_-])$`, 'gi');
const between = /^between$/gi;
const sqlStatements = /^delete$/gi;

const obtainType = function (token) {
    supportedFunctions.lastIndex = 0;
    between.lastIndex = 0;
    sqlStatements.lastIndex = 0;
    if (token[0] === 'LITERAL' && supportedFunctions.test(token[1])) {
        return 'FUNCTION';
